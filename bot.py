...
async def search_ml(client, query, limit=20):
    ...
    try:
        ...
        deals = [d for item in items if (d := parse_item(item))]
        log.info(f"  ✓ {query}: {len(items)} resultados, {len(deals)} con descuento")
        return deals
...
async def collect_all_deals():
    log.info("📦 Iniciando collect_all_deals...")
    all_deals = []
    async with httpx.AsyncClient() as client:
        for i in range(0, len(KEYWORDS), 5):
            batch = KEYWORDS[i:i+5]
            log.info(f"Lote {i//5 + 1}: {batch}")
            results = await asyncio.gather(*[search_ml(client, kw) for kw in batch])
            for r in results:
                all_deals.extend(r)
            await asyncio.sleep(0.3)

    log.info(f"Total antes de dedup: {len(all_deals)}")
    seen_titles = set()
    unique = []
    for d in all_deals:
        t = d["title"][:50].lower()
    ...
    log.info(f"Únicos con descuento ≥{DESCUENTO_MIN}%: {len(unique)}")
    return unique[:25]
...
async def send_deals(bot, chat_ids, deals):
    log.info(f"📤 Enviando {len(deals)} ofertas a {chat_ids}")
    if not deals:
        for cid in chat_ids:
            await bot.send_message(cid,
                "😔 No encontré ofertas con ≥40% de descuento ahora mismo.\n"
                "Mercado Libre tiene pocos productos con precio original visible hoy.\n"
        ...
        try:
            ...
            for i, deal in enumerate(deals, 1):
                try:
                    ...
                    await bot.send_message(cid, format_deal(deal, i),
                        parse_mode=ParseMode.MARKDOWN_V2, reply_markup=kb, disable_web_page_preview=True)
                    await asyncio.sleep(0.3)
                except Exception as e:
                    log.warning(f"Error oferta {i}: {e}")
            await bot.send_message(cid,
                f"✅ *Fin del reporte*\n⏰ Próxima búsqueda en *{INTERVALO_HORAS} horas*",
                parse_mode=ParseMode.MARKDOWN_V2)
        except Exception as e:
            log.error(f"Error enviando a {cid}: {e}")

async def job_buscar(bot):
    log.info("⏰ JOB INICIADO — buscando ofertas...")
    chat_ids = load_json(CHAT_ID_FILE, [])
    if not chat_ids:
        log.warning("Sin chat_ids — escríbele /start al bot primero.")
        return
    ...
    log.info(f"Nuevas: {len(new_deals)} / total: {len(deals)}")
    await send_deals(bot, chat_ids, new_deals)

async def cmd_start(update, ctx):
    cid = update.effective_chat.id
    log.info(f"/start desde chat_id={cid}")
    ...
    if cid not in ids:
        ...
        txt = (
            ...
            f"• Fuente: Mercado Libre MX\n\n"
            f"*Comandos:*\n/ofertas — Buscar ahora\n"
            f"/buscar laptop — Buscar producto\n"
            f"/estado — Ver estado\n/salir — Desactivar"
        )
    else:
    ...
    log.info(f"/start respondido a {cid}")

async def cmd_ofertas(update, ctx):
    cid = update.effective_chat.id
    log.info(f"/ofertas desde {cid}")
    await update.message.reply_text("🔍 Buscando ofertas... puede tardar ~30 segundos.")
    deals = await collect_all_deals()
...
async def cmd_buscar(update, ctx):
    ...
    if not query:
        ...
        return
    log.info(f"/buscar '{query}' desde {update.effective_chat.id}")
    ...
    if deals:
        ...
        await update.message.reply_text(
            f"😔 No encontré '{query}' con ≥40% descuento en $100–$6,000 MXN.\n"
...
async def cmd_estado(update, ctx):
    ...
    txt = (
        f"📊 *Estado del bot*\n━━━━━━━━━━━━━━━━\n"
        f"✅ Activo\n👥 Chats: *{len(ids)}*\n"
        f"📦 Historial: *{len(seen)} ofertas*\n"
        f"⏰ Intervalo: *{INTERVALO_HORAS}h*\n"
        f"💰 Rango: *\\$100 – \\$6,000 MXN*\n"
        f"🏷️ Desc\\. mín\\.: *40%*\n"
        f"🕐 {escape_md(datetime.datetime.now().strftime('%d/%m/%Y %H:%M'))}"
    )
    await update.message.reply_text(txt, parse_mode=ParseMode.MARKDOWN_V2)
